...
void main() 
{
    ...
    int i=0;
    OSCCON = 0x72;
    for(i=0;i<30;i++) mess[i]=' ';      /*initialize a string of spaces*/
    ...
    while(1)
    {
        ...
        if (key==' '){
            LCD_Clear();
            for(i=0;i<30;i++) mess[i]=' ';/*re-initialize a string of spaces*/
            i=0;
        }
        else if(key=='='){
            LCD_Clear();
            LCD_String_xy(0,0,"ANS: ");
            LCD_Command(0xC0);
            for(i=0;i<30;i++){
                LCD_Char(mess[i]);
            }
            ...
            i++;
            LCD_Char(key);   
        }               /* display pressed key on LCD16x2 */
    }
